    ...
    <div class="block-content boxes">
        <div class="row">
            <div class="col-sm-12">
                    ...
                    @if (!$data->isEmpty())
                    @foreach (break_array($data, 3) as $row)
                    <div class="row">
                    @foreach ($row as $single)
                        <div class="col-md-3">
                            <div class="box background-white">
                                <div class="box-picture">
                                    <a href="{{ url('ads/category/'.$single->id) }}">
                                        @if (!$single->img)
                                        <img src="{{ url('resize?w=320&amp;h=320&amp;r=1&amp;c=1&amp;src=media/default.png')}}" alt="{{ $single->title }}">
                                        @else
                                        <img src="{{ url('resize?w=320&amp;h=320&amp;r=1&amp;c=1&amp;src=media/categories/'.$single->img)}}" alt="{{ $single->title }}">
                                        @endif
                                        <span></span>
                                    </a>
                                </div><!-- /.box-picture -->
