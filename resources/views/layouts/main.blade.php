...
<body class="no-transition {{-- stretched --}} {{ str_replace('/', ' ',request()->path()) }} sticky-responsive-menu @if(getLang()=='ar') rtl @else ltr @endif">
<?php
  ...
  <div class="container">
    <div class="row y-header">
      <div class="col-xs-12 col-sm-9 col-md-10 col-lg-11">
          ...
          @if(Auth::user())
              @if(isset(Auth::user()->clinic))
                  @if(Auth::user()->clinic->logo!=null)
                      <a href="#"><img src="/media/clinics/{{Auth::user()->clinic->logo}}" width="150" alt="{{optional(Auth::user()->clinic)->title.LANG}}"></a>
                  @endif
              @endif
          @else
              @if(!session()->has('hidden_home'))
                <a href="{{ url('') }}"><img src="https://www.dietfix.com/dietfix-logo.png" width="200" alt="Diet Fix Logo"></a>
               @endif
          @endif

        </div>
        @if(request()->route()->getName()!='call_back_payment')
            ...
            @endif
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                  <ul class="nav navbar-nav">
                    ...
                    @if (Auth::user() && (Auth::user()->role_id == 1||Auth::user()->role_id == 20) )
                    ...
                    @if(session()->has('hidden_home'))
                        <li><a href="{{ url('user/newlogout') }}">{{ trans('main.Logout') }}</a></li>
                          @else
    ...
    @if(!session()->has('hidden_home') && request()->route()->getName()!='call_back_payment')
