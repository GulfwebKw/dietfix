...
@section('custom_head_include')
	...
	{{ HTML::script('cpassets/jsPDF-1.2.60/dist/jspdf.debug.js') }}
	@if (!empty($customJS))
...
jQuery(document).ready(function($) {       
   ...
   var table = $('#grid').DataTable({
        ...
        "columns": [
          ...
          { "sortable": true, "title": "{{ $field['name']}}" },
...
//          { "sortable": false, "data": "mainGridButsCol" } //MH
        ...
        "language": {
            ...
            "processing": '<i class="fa fa-coffee"></i>&nbsp;@lang('main.Please wait')...',
        ...
        "columnDefs": [{
        	"defaultContent": "<button onclick='showPrintDlg($(this))'>{{trans('main.Print Invoice')}}</button>",
    "targets": "_all",
            }
 ...
 //        $(this).html( '<div class="form-group"><input type="text" class="form-control search-field" placeholder="{{ trans('main.Search') }} '+title+'" /></div>' );
...
/*   @foreach ($buttons as $b)
   ...
   $('#grid_{{ strtolower($b['name']) }}').click(function  () {
   	...
   	var answer = confirm ( {{ Lang::get('main.GridDeleteConfirmation') }} );
