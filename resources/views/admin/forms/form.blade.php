...
@if(!empty($recordButsx))

<div class="btn-group pull-right" style="margin-top:58px;margin-right:20px;">

            @foreach ($recordButsx as $b)


                    <a style="cursor:pointer;" class="btn {{ $b['color'] }}"  href="{{url('admin/users_days/'.$b['link'].'/'.Request()->id)}}">

                        {{$b['name']}} <i class="fa fa-{{ $b['icon'] }}"></i>
...
@endif
	...
	<div class="form-actions">
		...
		{{ Form::button('<i class="fa fa-check"></i> '. trans('main.Save'), array('type' => 'submit', 'class' => 'btn col-md-4 yellow')) }}



		{{ Form::button('<i class="fa fa-reply"></i> '. trans('main.Save & Return'), array('type' => 'submit', 'name' => 'save_return', 'value' => '1' , 'class' => 'btn col-md-4 blue')) }}



		{{ Form::button('<i class="fa fa-plus"></i> '. trans('main.Save & New'), array('type' => 'submit', 'name' => 'save_new', 'value' => '1' , 'class' => 'btn col-md-4 green')) }}
