...
@section('contents')
	...
	{{ Form::open(['url' => url('schools/save') , 'files' => true , 'class' => 'form-horizontal form-bordered form-row-stripped spaceForm', 'role' => 'form']) }}
	...
	<div class="form-group">
		{{ Form::label('ages', trans('main.Grades') , array('class' => 'col-md-3 text-left')) }}
		<div class="col-md-9">
			<select id="ages" name="ages[]" class="form-control" multiple="multiple">
				@foreach ($ages as $k => $age)
			    <option value="{{ $k }}" @if (isset($item->ages) && in_array($k,array_fetch($item->ages->toArray(),'id'))) selected="selected" @endif >{{ $age }}</option>
				@endforeach
			</select>
		</div>
	</div>
	...
	<div class="form-group">
		{{ Form::label('genres', trans('main.Genders') , array('class' => 'col-md-3 text-left')) }}
		<div class="col-md-9">
			<select id="genres" name="genres[]" class="form-control" multiple="multiple">
				@foreach ($genres as $k => $genre)
			    <option value="{{ $k }}" @if (isset($item->genres) && in_array($k,array_fetch($item->genres->toArray(),'id'))) selected="selected" @endif >{{ $genre }}</option>
				@endforeach
			</select>
		</div>
	</div>
