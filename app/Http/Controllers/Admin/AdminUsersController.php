    ...
    public function saveOrder(Request $request)
    {
        ...
        if(isset($userDate->package_id)){
           ...
           //check meals counts and user chosen meals count
            ...
            if (!empty($items) && count($items) != $totalMealsByPackage) {
                return response()->json(['result'=>false]);
            }
            ...
            if($user->package_id!=$request->package_id){
                ...
                if($forFuture && $countDay>0){
                    ...
                    if(isset($request->attach_day)){
                        $this->makeAdminLog("Admin: in handelAddOrNewMembership startDate=$firstDate select attach days option  count days ==>$countDay   userId==>$uId ",null,$uId," handelAddOrNewMembership");
...
//try{
    ...
    $titleEn = !empty($request->title_en)?$request->title_en:'DietFix Notification';
	...
	public function sendPushNotification($titleAr,$titleEn,$contentAr,$contentEn,$users)
    {
        ...
        if(!empty($users->id)){
          ...
          array_push($arrayToken,$users->deviceToken);
        }
    ...
    {
        $user=User::find($id);
		$membershipDate = !empty($user->membership_start)?$user->membership_start:date('Y-m-d');
        $countExistFreeze=UserDate::where('user_id',$user->id)->where('date','>=',$membershipDate)->where('freeze',1)->count();
    ...
    public function freezeDays(Request $request)
    {
        ...
        $id=$user->id;
        $this->makeAdminLog("Admin:freeze user days from $starting_date  userId===>$id ",null,$id,"freeze user days");
