...
class UserController extends MainApiController
{
    ...
    public function getUserDetail(Request $request)
    {
        ...
        return $this->sendResponse(200,['data'=>['user'=>$user,'remind_day'=>$this->getCountRemDaysUser($user->id,$user->membership_start),'sum_point'=>$res[0],'count_day'=>$res[1],'sum_cash_back'=>$res[2],'subscription_end_date'=>$lastDate,'is_future_subscription'=>$is_future_subscription,'new_starting_date'=>$newSubscriptionDate],'message'=>'']);
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            return $this->sendResponse(200, ['data' => ['user' => $user,'remind_day'=>$this->getCountRemDaysUser($user->id,$user->membership_start),'sum_point'=>$res[0],'count_day'=>$res[1],'sum_cash_back'=>$res[2]], 'message' => trans('main.update_personal_success')]);
        }catch (\Exception $e){
...
//                    }
                    ...
                    return $this->sendResponse(200,['data'=>['user'=>$user,'remind_day'=>$this->getCountRemDaysUser($user->id,$user->membership_start),'sum_point'=>$res[0],'count_day'=>$res[1],'sum_cash_back'=>$res[2]],'message'=>trans('main.update_information')]);
    ...
    public function updatePhoto(Request $request,Filesystem $filesystem)
    {
        ...
        try{
            ...
            if(isset($user)){
                ...
                return $this->sendResponse(200,['data'=>['user'=>$user,'remind_day'=>$this->getCountRemDaysUser($user->id,$user->membership_start),'sum_point'=>$res[0],'count_day'=>$res[1]],'sum_cash_back'=>$res[2],'message'=>trans('update_profile')]);
            }
...
//                        $res= DB::table('orders')->where('user_id',$user->id)->where('day_id',$dayId->id)->where('meal_id',$maleArray['male_id'])->select('*')->first();
//                        if(isset($res)&& $res!=null){
//                              DB::table('orders')->where('user_id',$user->id)->where('day_id',$dayId->id)->where('meal_id',$maleArray['male_id'])->update(['category_id'=>$maleArray['category_id'],'item_id'=>$maleArray['item_id']]);
//                              $orderId=$res->id;
//                        }else{
                                $order=new Order();
                        ...
                        // $orderId=  DB::table('orders')->insert(['category_id'=>$maleArray['category_id'],'item_id'=>$maleArray['item_id'],'meal_id'=>$maleArray['male_id'],'date_id'=>$day->id,'user_id'=>$user->id,'day_id'=>optional($dayId)->id]);
//                        }
    ...
    public function chooseRandomFood(Request $request)
    {
        ...
        try{
            ...
            $package=Package::where('id',$user->package_id)->where('show_mobile',1)->where('active',1)->with(['meals'=>function($q){$q->where('active',1);}])->first();
                ...
                if(isset($package->meals)){
                    foreach ($package->meals as $meal) {
                        ...
                        if(isset($res) && !empty($res)){
                            ...
                            $this->logUserActivity("choseRandom food  for date ".$userDate->date." and meal==>".$meal->id."  userId==>".$user->id,null,$user->deviceType);
    ...
    public function freezeDay(Request $request)
    {
       ...
       try{
           ...
           $firstValidDay=$request->start_day;
            $user=$this->getUser($request);
             //            $date_from =$request->start_day;
        //            $date_from = strtotime($date_from); // Convert date to a UNIX timestamp
        //            // Specify the end date. This date can be any English textual format
        //            $date_to = $request->end_day;
        ...
        //            }
              ...
              //    $existFreeze=UserDate::where('user_id',$user->id)->whereIn('date',$days)->count();
              //            if($existFreeze>=1){
             //                return  $this->sendResponse(205,['data'=>[],'message'=>"You already freeze days"]);
             //            }
            ...
            if($res){
//                if($contFreeze>=1){
//                    $lastDay=UserDate::where('user_id',$user->id)->where('freeze',0)->orderBy('date','desc')->select(['date'])->first();
//                    $lastDayUser=strtotime($lastDay->date);
//                      $start=$lastDayUser;
//                        for($j=1;$j<=$contFreeze;$j++){
//                            $start=$start+86400;
//                            $day=$start;
//                            UserDate::create(['user_id'=>$user->id,'date'=>date("Y-m-d",$day)]);
//                        }
//                }
    ...
    public function cancelFreezeDay(Request $request)
    {
        try{
            ...
            //$countExistFreeze=UserDate::where('user_id',$user->id)->where('date','>=',$user->membership_start)->where('freeze',1)->where('date','>=',$firstValidDay)->count();
            $countExistFreeze=UserDate::where('user_id',$user->id)->where('date','>=',$user->membership_start)->where('freeze',1)->count();

            if(intval($countExistFreeze)<=0){
                return  $this->sendResponse(205,['data'=>[],'message'=>trans('main.Not_Found_Freeze_Day')]);
            }

            $idS=UserDate::where('user_id',$user->id)->where('date','>=',$user->membership_start)->where('freeze',1)->pluck('id');

            UserDate::where('user_id',$user->id)->where('date','>=',$user->membership_start)->where('freeze',1)->delete();
            Order::where('user_id',$user->id)->whereIn('date_id',$idS)->delete();

            $this->logUserActivity("Dates & Orders are removed due to cancel freeze days  userId==>".$user->id,null,$user->deviceType);
    ...
    public function helpMeChoosePackage(Request $request)
    {
        ...
        try{
           // $lisPackage=Package::whereIn('titleEn',['Full Plus','Full','Fitness','Vegan','Detox','Atkins','Keto'])->where('active',1)->where('show_mobile',1)->get()->keyBy('titleEn')->toArray();
            $lisPackage=Package::whereIn('titleEn',['Full Plus','Full','Sports','Vegan','Detox','Atkins','Keto'])->where('active',1)->where('show_mobile',1)->get()->keyBy('titleEn')->toArray();
...
//
    ...
    public function setDay(Request $request)
    {
        try{
			...
			if(!empty($user->membership_start) && !empty($request->day) && $request->day < $user->membership_start){
			return $this->sendResponse(400,['data'=>[],'message'=>"Chosen date is invalid"]);
			}
...
//        },'packaged'])->select(['id','paymentID','ref_id','paid','package_id','package_duration_id','PaidCurrencyValue','DueValue','total','PaidCurrency','Currency','CustomerServiceCharge','type','status','created_at','updated_at'])->orderBy('id','desc')->limit(100)->get();
    ...
    private function getListUserDays($userId,$statingDate=null)
    {
        ...
        //  $res=$res->get()->toArray();
        //   $orders=DB::table('orders')->where('user_id',$userId)->select(['date_id'])->get()->keyBy('date_id');
...
//              $res=$res->where('date','>=',$memberShipStart);
//          }
          return $res->count();
